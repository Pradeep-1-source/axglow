...
// ─────────────────────────────────────────────────────────────
// AXGLOW — locked scroll-scrub video hero
...
export interface MetroHeroProps {
  ...
  /** Called once when the video scrub reaches the end (progress >= 0.98). */
  onComplete?: () => void
  className?: string
...
const DEFAULT_SIGNATURE = { name: "AXGLOW", url: "#" }
const SANS = "'Space Grotesk', 'Plus Jakarta Sans', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif"

const COL_BG = "#05070d"
const COL_TEXT = "#f2f4f8"
const COL_BRAND = "#3BD8D9"
...
export default function MetroHero({
  ...
  useEffect(() => {
    ...
    function frame() {
      ...
      // Fire onComplete once when scrub reaches near-end
      if (!completeFired && currentProgress >= 0.98 && onComplete) {
        completeFired = true
        releaseLock()
        onComplete()
      }
  ...
  return (
    ...
    >
      ...
      >
        <span
          style={{
            ...
            background: `linear-gradient(135deg, ${COL_BRAND} 0%, #FFFFFF 50%, #8A46BB 100%)`,
            WebkitBackgroundClip: "text",
            WebkitTextFillColor: "transparent",
            backgroundClip: "text",
            textShadow: "none",
            filter: `drop-shadow(0 4px 30px rgba(59,216,217,0.4))`,
      ...
      {/* Thin progress line — fills as the video advances, using brand cyan gradient. */}
      <div
        style={{
          position: "absolute",
          left: 0,
          ...
          bottom: 0,
          height: 2,
          background: "rgba(59,216,217,0.12)",
        }}
      >
        <div
          ref={progressBarRef}
          style={{
            height: "100%",
            width: "100%",
            background: `linear-gradient(90deg, ${COL_BRAND}, #8A46BB)`,
            transform: "scaleX(0)",
        ...
        >
          ...
          <a
            ...
            onMouseEnter={(e: React.MouseEvent<HTMLAnchorElement>) => {
              e.currentTarget.style.color = COL_BRAND
            }}
            onMouseLeave={(e: React.MouseEvent<HTMLAnchorElement>) => {
              e.currentTarget.style.color = "rgba(59,216,217,0.5)"
            }}
          >
            {signature.name}
          </a>
